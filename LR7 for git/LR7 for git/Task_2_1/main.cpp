...
int main() {
    ...
    while (1)
    {
        ...
        std::string a = toDop(makeEq(num1, k1)), b = toDop(makeEq(num2, k2));//O(n)
        std::string c = sum(a,b); //O(n) 

        std::cout << "Числа в допкоде: " << "\n" << a << "\n" << b << "\n"; //O(n)
        std::cout << "Сумма в допкоде: " << "\n" << c << "\n"; //O(n)
